                ...
                <td class="sm-p-6" style="border-radius: 8px; background-color: #fffffe; padding: 24px 36px; border: 1px solid #e2e8f0">
                  ...
                  <div>
                    <h1 style="margin: 0; font-size: 24px; line-height: 32px; font-weight: 600; color: #264054">
                      Sveiki, {{ $invoice->owner->first_name }},
                    </h1>
                    <p style="font-size: 16px; line-height: 24px; color: #366c93; margin: 24px 0 0">
                      Pateikiame jums informacinę sąskaitą faktūrą už {{$dateName}}.
                    </p>
                    <ul style="list-style-type: disc; margin-top: 24px; margin-bottom: 0">
                      <li>
                        <strong>Sąskaitos numeris:</strong> SS {{ $invoice->number }}
                      </li>
                      <li>
                        <strong>Laikotarpis:</strong> {{\Carbon\Carbon::parse($invoice->date)->startOfMonth()->format('Y-m-d')}} – {{ $invoice->date }}
                      </li>
                      <li>
                        <strong>Bendra suma:</strong> {{ $invoice->price_with_vat }} €
                      </li>
                      <li>
                        <strong>Išrašymo data:</strong> {{ $invoice->date }}
                      </li>
                    </ul>
                    <div style="margin-top: 24px; margin-bottom: 0">
                      <a href="{{ $appUrl }}/dashboard/invoices" style="display: inline-block; text-decoration: none; padding: 16px 24px; font-size: 16px; line-height: 1; border-radius: 4px; color: #fffffe; background-color: #2f5c7e" class="hover-bg-blumine-800">
